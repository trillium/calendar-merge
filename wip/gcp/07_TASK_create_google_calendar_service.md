# Task 07: Create Google Calendar Service

**Status:** Not Started
**Priority:** Critical
**Estimated Time:** 2-3 hours
**Dependencies:** Task 06 (Google Auth Service)

---

## Objective

Create a service for interacting with the Google Calendar API (list calendars, get events, create/update/delete events, etc.).

## Why This Task?

- Provides abstraction layer over Google Calendar API
- Used by event sync, batch sync, and watch services
- Centralizes all Calendar API calls

## Target File

```
gcp/src/services/
└── google-calendar.service.ts  (Google Calendar API operations)
```

## Steps

### 1. Create google-calendar.service.ts
...
- [ ] google-calendar.service.ts created
- [ ] All Google Calendar API operations implemented
- [ ] Error handling for common API errors (404, 410, etc.)
- [ ] Rate limiting helper included
...
```typescript
// gcp/src/services/test-calendar.ts (temporary)
import { listCalendars } from './google-calendar.service';
...
async function testCalendar() {
  try {
    ...
    console.log('\nCalendar service test successful!');
  } catch (error) {
...
→ **08_TASK_create_event_sync_service.md** - Port sync.ts logic (includes Airbnb feature)

## Notes

- This service wraps the googleapis Calendar API
- All functions take `userId` first for consistent auth
- Rate limiting is built-in via `withRateLimit()`
- Watch operations are included for webhook setup
- 410 errors (expired syncToken) are handled specially
